...
def dynamic_instructions(ctx:RunContextWrapper[Passsenger],agent: Agent):
    if ctx.context.user_type == "Window" and ctx.context.travelling_experience == "First time":
        return "Explain window benefits, mention scenic views, reassure about flight experience."
    elif ctx.context.user_type == "Aisle" and ctx.context.travelling_experience == "Occasional":
    ...
    elif ctx.context.user_type == "Middle" and ctx.context.travelling_experience == "Frequent":
        return "Acknowledge the compromise, suggest strategies, offer alternatives."
    elif ctx.context.user_type == "Any" and ctx.context.travelling_experience == "Premium":
...
airline_booking = Agent(
    ...
    instructions= f"you are helpful airline seat preference agent. {dynamic_instructions}"
)
